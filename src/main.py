...
def main(config):
    ...
    # flops = FlopCountAnalysis(model, torch.rand(1,100,17).to(device))
    # logger.info(flops.total())
    # print('Total FLOPs: ',flops.total())

    test_results_filepath = os.path.join(config['output_dir'], 'test_results')
    np.savez(test_results_filepath, **per_batch_test)
    return best_value
