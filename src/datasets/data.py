...
class RobotData(BaseData):
    ...
    def load_all(self, root_dir, file_list=None, pattern=None):
        ...
        input_paths = [p for p in selected_paths if os.path.isfile(p) and p.endswith('.mat')]
        if len(input_paths) == 0:
            raise Exception("No .ts files found using pattern: '{}'".format(pattern))
...
#             all_df_new, labels_df_new = self.load_single(input_paths[i+1])
#             all_df = pd.concat([all_df,all_df_new],ignore_index=True)
#             labels_df = pd.concat([labels_df, labels_df_new],ignore_index=True)
            
#             print(all_df)
#             print(labels_df)
            
        all_df, labels_df = self.load_files(input_paths)
    ...
    def load_files(self, input_paths):
        ...
        if self.config['task'] == 'regression':
            ...
            # df = pd.DataFrame(columns=['dim_0','dim_1','dim_2','dim_3','dim_4','dim_5','dim_6','dim_7','dim_8','dim_9'
            #                           ,'dim_10','dim_11','dim_12','dim_13','dim_14','dim_15','dim_16','dim_17','dim_18'
            #                           ,'dim_19','dim_20','dim_21','dim_22'])
            
            # df = pd.DataFrame(columns=['dim_0','dim_1','dim_2','dim_3','dim_4','dim_5','dim_6','dim_7','dim_8','dim_9'
            #                           ,'dim_10','dim_11','dim_12','dim_13','dim_14','dim_15','dim_16'])
            ...
            for filepath in input_paths:
                ...
                data = mat['data'][:,0:mat['data'].shape[1]-1]
                # data = mat['data'][:,0:3]
                originalLabels = mat['data'][:,mat['data'].shape[1]-1]
                for i in range(math.floor(len(data)/fragmentLength)):
                    seriesList = [];
                    #rand_num = random.randint(0,50)
                    rand_num = 0
                    for j in range(mat['data'].shape[1]-1):
                    # for j in range(3):
                        seriesList.append(pd.Series(data[i*fragmentLength:(i+1)*fragmentLength-rand_num,j]))
                    df.loc[len(df.index)] = seriesList
                    labels.append(int(stats.mode(originalLabels[i*fragmentLength:(i+1)*fragmentLength])[0]));
                print(df.shape)
                print(len(labels))
